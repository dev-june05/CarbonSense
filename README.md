...
Predictions are great, but enterprises need audits. Clicking the **Burn Test** locks your code in a local sandbox and executes it. Using `CodeCarbon`, it measures the physical Joules of electricity pulled from your specific CPU/GPU architecture (Intel/AMD) and returns a mathematically verified Carbon Receipt.
...
Because CarbonSense bridges a Node.js frontend with a Python ML backend, you must run both environments.

### 1. Start the Telemetry Backend
Navigate to the backend folder and start the AI engine:
\`\`\`bash
cd backend
python -m venv .venv
